...
like butterscotch ice-cream. Write C++ program to store two sets using linked list. compute and display- 
...
{   node *p=new(struct node);
  ...
  void info::insert()
  {
   ...
   bool info::isPresent(int x)
    {
        temp = head;
        while (temp) {
            if (temp->roll == x) return true;
            temp = temp->next;
        }
        return false;
    }
